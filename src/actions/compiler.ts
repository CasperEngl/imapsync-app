    ...
    try {
      ...
      for (const input of state.transfer.inputs) {
        if (input) {
          ...
          if (
            (validator.isURL(host_1) || validator.isIP(host_1))
            && (validator.isURL(host_2) || validator.isIP(host_2))
            && !validator.isEmpty(user_1)
            && !validator.isEmpty(user_2)
            && password_1 !== 'PASSWORD_1'
            && !validator.isEmpty(password_1)
            && password_2 !== 'PASSWORD_2'
            && !validator.isEmpty(password_2)
          ) {
            ...
            const commands = newCommand
              .split(' ')
              .filter((arg) => arg.length || !arg.includes('./imapsync_bin'))
              .map((arg) => arg
                .replace(/;$/, '')
                .replace(/^'(.*)'$/, '$1')
                .trim(),
